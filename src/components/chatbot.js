        ...
        "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyBPk4NYw_5enfX5-OJxYN14haaxFoQxiPM",
        {
          ...
          body: JSON.stringify({
            contents: [
              {
                parts: [{ 
                  text: `Respond to the user query: ${val}\n\nDo NOT use markdown format! Respond from the point of view of a chill and professional co-worker.  Only respond in plain text with NO formatting. Respond with SHORT, readable messages. The user is working on an unprocessed pallet survey for their job. This survey requires entries for remaining pallets, cartons processed, CPH, ZPH, and pallets delivered. The specialized calculator they are using will do the calculations for them as long as they have data for carried-over pallets, remaining pallets, pallets delivered, hours hanging, z-racks filled, and total backroom hours. The user might also want to chat about general practices for running a backroom at a retail store. Here is more info the user might need ONLY if they ask for it. Track z-racks filled and hours hanging by having associates record the z-rack tracking forms, which are located by each hanging station. Calculate total hours worked by adding all hours recorded on the z-rack tracking forms. It is important to check in with associates daily to ensure they are recording their z-rack tracking information and their hours worked. this data throughout the week to ensure the correct data is being entered into the unprocessed survey. 
` 
                }]
              }
            ]
